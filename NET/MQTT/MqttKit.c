/**
  ...
  * 说明        MQTT协议
  *
  * 修改记录    V1.1 修改MQTT_PacketSubscribe的参数为2个topic
  *                    解决当时协议栈bug
  *              V1.2 修改MQTT_PacketCmdResp的bug
  *              V1.3 用strncpy替换为memcpy，解决编译bug
  *              V1.4 修改 MQTT_PacketPublishAck
  *                     MQTT_PacketPublishRel
  *                     两个函数的参数bug
  *              V1.5 修改 MQTT_UnPacketCmd
...
uint8 MQTT_UnPacketRecv(uint8 *dataPtr)
{
	...
	if(type == MQTT_PKT_PUBLISH)
	{
		...
		if(strstr((int8 *)msgPtr + 2, CMD_TOPIC_PREFIX) != NULL)	// 判断是否为命令下发
			status = MQTT_PKT_CMD;
		else
							...
							MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	// qos级别--主要用于PUBLISH发布态消息的，保证消息传递的次数-----------------------------
	switch((unsigned char)qos)
	{
		case MQTT_QOS_LEVEL0:
			flags |= MQTT_CONNECT_WILL_QOS0;							// 最多发一次
		break;
		...
		case MQTT_QOS_LEVEL2:
			flags |= (MQTT_CONNECT_WILL_FLAG | MQTT_CONNECT_WILL_QOS2);	// 只发一次
		break;
	...
	// 需要保留PUBLISH(发布态)消息，表示服务器需要保留这个消息，如果新的订阅者出现，就把这个消息发送给这个订阅者，这样订阅者就不用等待发送端发布
	if(will_retain)
	{
...
uint1 MQTT_PacketSaveData(const int8 *pro_id, const char *dev_name,
									int16 send_len, int8 *data_buf, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	if(MQTT_PacketPublish(MQTT_PUBLISH_ID, topic_buf, (const int8 *)data_buf, send_len, MQTT_QOS_LEVEL0, 0, 1, mqttPacket) == 0)
		return 0;
	else
		return 1;
...
uint8 MQTT_UnPacketCmd(uint8 *rev_data, int8 **cmdid, int8 **req, uint16 *req_len)
{
	...
	int8 *dataPtr = strchr((int8 *)rev_data + 6, '/');	// 第6个为头信息
	...
	*cmdid = (int8 *)MQTT_MallocBuffer(37);				// cmdid固定36字节，再加一个字符串结束位
	if(*cmdid == NULL)
		return 2;

	memset(*cmdid, 0, 37);								// 全部清零
	memcpy(*cmdid, (const int8 *)dataPtr, 36);				// 提取cmdid
	dataPtr += 36;
	...
	*req = (int8 *)MQTT_MallocBuffer(*req_len + 1);		// 分配命令长度+1
	...
	memcpy(*req, (const int8 *)dataPtr, *req_len);			// 拷贝命令
	return 0;
...
//
//  函数功能    Subscribe的回复信息解包
//
//  入口参数    rev_data-接收的消息信息
//
//  返回参数    0-成功        其他-失败
//
//  说明
	...
	switch(*qos)
	{
		case MQTT_QOS_LEVEL0:									// qos0 have no packet identifier
			...
			*topic = MQTT_MallocBuffer(*topic_len + 1);			// 为topic分配内存
			...
			memcpy(*topic, (int8 *)msgPtr + 2, *topic_len);		// 拷贝数据
			...
			*payload = MQTT_MallocBuffer(*payload_len + 1);
			if(*payload == NULL)								// 若失败
			{
				MQTT_FreeBuffer(*topic);						// 需要把topic的内存也释放掉
				return 255;
			}
...
//  返回参数    0-成功      1-失败原因
//
//  说明        接收到Publish消息且QoS等级为1时，需要Ack回复
//==========================================================
...
//  返回参数    0-成功      1-失败原因
//
//  说明        接收到Publish消息且QoS等级为2时，接收到rec
//==========================================================
...
//  返回参数    0-成功      1-失败原因
//
//  说明        接收到Publish消息且QoS等级为2时，接收到rec再回复rel
//==========================================================
